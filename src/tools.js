// Shared MCP tool definitions for the agadmator catalogue.
// Both transports register identical tools via registerTools(server):
//   - src/server.js          → stdio (local dev, Claude Desktop)
//   - web/app/api/[transport] → Streamable HTTP (remote, Vercel)
// Keeping the schemas here guarantees the remote server exposes the exact
// same tools as the local one.

import { z } from "zod";
import { searchGames, getGame } from "./search.js";

const json = (data) => ({ content: [{ type: "text", text: JSON.stringify(data) }] });

const searchGamesSchema = {
...
/**
 * Register the agadmator tools on an McpServer instance. Transport-agnostic:
 * the same call wires up the stdio server and the HTTP route handler.
